...
from fitbit_uitls.fitbit_funcs import get_last_min_heart_data, get_live_heart_rate, get_activity_data
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([f"{usr_data['name']}'s Home", "Contact Medical Professional", "Fitbit Logger", "Activity Logger", "Logout"])
    ...
    with tab4:
        st.image("https://cdn.iconscout.com/icon/free/png-256/free-fitbit-282220.png", width=200)
        st.subheader("Add your Fitbit API Refresh Token and Fetch Activity Data")
        with open("./config.json") as f:
            data = json.load(f)
            access_token = data.get("access_token")

        fitbit_key = st.text_input(label="Fitbit Access Token", value=access_token)
        btn_activity = st.button("Fetch Activity Data")

        if btn_activity:
            st.session_state['fitbit_key'] = fitbit_key

            activity_df = get_activity_data(access_token=fitbit_key, date=DATE)

            if os.path.exists("activity_data.csv"):
                existing_df = pd.read_csv("activity_data.csv", index_col=0)
                activity_df = pd.concat([existing_df, activity_df])

            activity_df.to_csv(f"{usr_data['username']}_activity_data.csv")
